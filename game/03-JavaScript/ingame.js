...
const combatActionColours = {
	Default: {
		...
		def: [
			...
			"kick",
			/* mouthaction */
			"bite", "demand", "breastbite", "handbite", "headbutt", "bitepussy", "fengMing",
		],
		meek: [
			/* leftaction or rightaction */
...
window.DefaultActions = {
	...
	setDefaults() {
		...
		this.addMany(type, "Defiant", "mouthaction", ["pullaway", "bite", "breastbite", "headbutt", "fengMing"]);
...
function currentSkillValue(skill) {
	...
	switch (skill) {
		case "skulduggery":
			...
			if (V.fenghuang >= 6) modifier += 0.10;
